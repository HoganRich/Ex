...
def sendmail(request):
    ...
    if not get_has_sentregemail(to_email):  # 检查用户是否在同一时间多次点击发送邮件
        ...
        url = "http://127.0.0.1:8000" + '/auth/email_notify?email=' + to_email + '&sign=' + sign  # 回调函数
...
def reset_password(request):
    ...
    if not get_has_sentemail(email):
        ...
        url = "http://127.0.0.1:8000" + '/auth/reset_notify?email=' + email + '&sign=' + sign + '&is_biz=' + str(is_biz)
